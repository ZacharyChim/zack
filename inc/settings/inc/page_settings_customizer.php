	...
	function customize_register( $wp_customize ){
		...
		if( method_exists($wp_customize, 'add_panel') ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'zack' ),
				'description' => __( 'Change layouts for various pages on your site.', 'zack' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'zack' ),
			'search' => __( 'Search Results', 'zack' ),
			'date' => __( 'Date Archives', 'zack' ),
			'author' => __( 'Author Archives', 'zack' ),
			'404' => __( 'Not Found', 'zack' ),
		);
		...
		foreach( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'zack' ) . ': ' . $post_type_data->label
			);
		}
		...
		foreach( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'zack' ) . ': ' . $taxonomy->label
			);
		}
